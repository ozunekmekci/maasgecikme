...
export const InterestTable: React.FC<InterestTableProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-2.5 py-1 rounded text-xs font-bold transition shadow-sm ${
                  isTcmbGradualMode
              ...
              >
                ...
                <span>{isTcmbGradualMode ? '⚡ TCMB Kademeli Aktif' : 'TCMB Kademeli Faizi Aç'}</span>
              </button>
            </div>
            ...
            {!isTcmbGradualMode ? (
              ...
              <div className="p-2 rounded bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-xs text-rose-800 dark:text-rose-300">
                <span className="font-bold block">TCMB En Yüksek Mevduat Oranları Devrede:</span>
                <span className="text-[11px] text-slate-600 dark:text-slate-400 block">
                  Her ay kendi muacceliyet dönemine ait resmi faizle (örn: %53.20, %52.80...) hesaplanıyor.
                </span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                className="px-2.5 py-1.5 text-xs font-bold rounded border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-mono flex-1"
              />
              <button
                onClick={setDateToday}
                className="px-3 py-1.5 rounded text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 transition flex items-center space-x-1 shrink-0"
                title="Bugünün Türkiye Tarihine Getir"
              >
          ...
          <div className="space-y-1.5">
            <label className="flex items-center space-x-1.5 text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
              ...
              <span>Muacceliyet Kuralı</span>
            </label>
            <div className="px-3 py-2 rounded border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-bold flex items-center justify-between">
              <span>Takip Eden Ayın {dueDay}'i</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-700 font-mono text-slate-600 dark:text-slate-300 font-semibold">
                İş Kanunu m.34
              </span>
            </div>
          </div>
      ...
      <div className={`bg-white dark:bg-slate-900 rounded-lg border shadow-sm overflow-hidden font-sans transition-colors ${
        isTcmbGradualMode ? 'border-rose-400 dark:border-rose-800 ring-1 ring-rose-300 dark:ring-rose-900' : 'border-slate-300 dark:border-slate-700'
      }`}>
        
        {/* Table Header Bar */}
        <div className={`p-3 sm:p-3.5 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-2 transition-colors ${
          isTcmbGradualMode 
        ...
        {/* TCMB Active Prominent Banner */}
        {isTcmbGradualMode && (
          <div className="p-3 bg-gradient-to-r from-rose-50 via-amber-50 to-rose-50 dark:from-rose-950/50 dark:via-slate-900 dark:to-rose-950/50 border-b border-rose-300 dark:border-rose-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
            <div className="flex items-center space-x-2">
              <span className="w-2.5 h-2.5 rounded-full bg-rose-600 animate-pulse shrink-0" />
              <span className="font-black text-rose-900 dark:text-rose-200 uppercase tracking-wide">
                ⚡ TCMB DÖNEMSEL KADEMELİ FAİZ AKTİF
              </span>
              <span className="text-slate-700 dark:text-slate-300 hidden md:inline">
                — Tablodaki her dönemin faizi resmi TCMB 1-3 aylık mevduat ortalamaları ile hesaplanmaktadır.
              </span>
            </div>
            <span className="px-2 py-0.5 rounded bg-rose-600 text-white font-mono font-bold text-[10px] uppercase shrink-0">
              İş Kanunu m.34 Uyumlu
            </span>
          </div>
        )}
          ...
          <table className="w-full text-left text-xs border-collapse border border-slate-300 dark:border-slate-700 min-w-[720px] sm:min-w-full">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-bold border-b border-slate-300 dark:border-slate-700 text-[11px] uppercase">
                ...
                <th className={`p-2 border border-slate-300 dark:border-slate-700 text-center transition-colors ${
                  isTcmbGradualMode ? 'bg-rose-100 dark:bg-rose-950/90 text-rose-900 dark:text-rose-200' : ''
                }`}>
                  {isTcmbGradualMode ? 'Yıllık Faiz (TCMB %)' : 'Yıllık Faiz (%)'}
                </th>
                    ...
                    <td className="p-2 border border-slate-300 dark:border-slate-700 text-center font-mono whitespace-nowrap">
                      {isEditing ? (
                        <input
                          ...
                          className="px-1 py-0.5 text-xs text-center rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 w-16 font-mono font-bold"
                        />
                      ) : isTcmbGradualMode ? (
                        <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-[11px] font-black bg-rose-100 dark:bg-rose-950/80 text-rose-800 dark:text-rose-300 border border-rose-300 dark:border-rose-800 shadow-sm" title={`TCMB 1-3 Ay Mevduat: %${row.annualInterestRate.toFixed(2)}`}>
                          <span>TCMB</span>
                          <span>%{row.annualInterestRate.toFixed(2)}</span>
                        </span>
                      ) : (
                        <span className="font-semibold text-slate-800 dark:text-slate-200">%{row.annualInterestRate}</span>
                      )}
                    </td>
